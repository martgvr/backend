...
import { ordersDAO } from "./daos/factory.js"
...
import { ordersModel } from "./models/sqlite/orders.sqlite.model.js"
...
export async function createSQLiteTables() {
    ...
    const ordersTable = await ordersDAO.createTable(ordersModel)

    logger.info(`SQLite Tables: [ ${cartsTable.tableName} = ${cartsTable.tableStatus === 1 ? `WORKING` : `ERROR`} | ${messagesTable.tableName} = ${messagesTable.tableStatus === 1 ? `WORKING` : `ERROR`} | ${productsTable.tableName} = ${productsTable.tableStatus === 1 ? `WORKING` : `ERROR`} | ${usersTable.tableName} = ${usersTable.tableStatus === 1 ? `WORKING` : `ERROR`} | ${ordersTable.tableName} = ${ordersTable.tableStatus === 1 ? `WORKING` : `ERROR`} ]`);
